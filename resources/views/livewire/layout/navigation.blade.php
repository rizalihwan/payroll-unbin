...
new class extends Component {
    ...
    public function mount()
    {
        ...
        $superior = Superior::where('user_id', $getUser->id)->first();
        $employee = Employee::where('user_id', $getUser->id)->first();

        if ($getUser->role == 0) {
            $this->superior = $superior->first_name . ' ' . $superior->last_name . ' (' . $superior->user->nip . ')' ?? null;
        } else {
            $this->employee = $employee->first_name . ' ' . $employee->last_name . ' (' . $employee->user->nip . ')' ?? null;
        }
    }
}; ?>
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button
                            ...
                            @php
                                $name = auth()->user()->role == 0 ? $superior : $employee;
                            @endphp

                            <div x-data="{ name: '{{ $name }}' }" x-text="name"
